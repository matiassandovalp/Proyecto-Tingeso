...
const ReservaForm = () => {
  // Estado para controlar el paso actual: 1 = formulario; 2 = confirmación/descarga
  ...
  const crearReserva = async (e) => {
    ...
    try {
      ...
      // Cambiamos al paso 2 para mostrar la confirmación
      setStep(2);
    } catch (error) {
  ...
  // Vista del paso 2: Confirmación y descarga de comprobante
  if (step === 2 && reservaCreada) {
    return (
      <Paper sx={{ p: 3, m: 3, textAlign: "center" }}>
        <Typography variant="h5">¡Reserva Creada con Éxito!</Typography>
        <Typography sx={{ mt: 2 }}>
          Tu reserva (ID: {reservaCreada.reservaId}) se ha realizado correctamente.
        </Typography>
        <Box sx={{ mt: 3 }}>
          {reservaCreada.comprobante ? (
            <ComprobanteButton
  ...
  // Vista del paso 1: Formulario de reserva
  return (
    <Paper sx={{ p: 3, m: 3 }}>
